...
def main():
    ...
    parser.add_argument("--M", type=int, nargs="+", default=[25_000], help="One or more M values (space separated)")
    parser.add_argument("--dt", type=float, default=0.1, help="Time step Δt")
    parser.add_argument("--A", type=float, nargs="+", default=[i/50.0 for i in range(0,51)], help="One or more A values (space separated)")
    ...
    if args.randomJ == 'True':
        J_base = create_random_J(args.N, seed=args.seed)
    else:
        J_base, N_base = read_matrix_from_file(args.file_path)
    print(J_base)
